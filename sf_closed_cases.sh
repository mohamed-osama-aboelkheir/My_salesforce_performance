...
\n\
DESCRIPTION:\n\
Used to List the cases closed by user in a specific day\n\
\n\
OPTIONS:\n\
-d: detailed output\n\
-u: print usage\n\
...
curl -s ${REST_URL}query/?q=SELECT+id,casenumber,closeddate+From+case+where+ownerid=\'$USER_ID\'+and+closeddate\>$START+and+closeddate\<$END -H "Authorization: Bearer $SESSION_ID" | sed "s/}/}\n/g" > $TMP_FILE 

cat $TMP_FILE | grep "CaseNumber" | wc -l
